...
*   [سرویس پوش‌ناتیفیکیشن با Node.js](#node-service)
    *   [آماده‌سازی سرویس Node](#prepare-node-service)
    *   [استفاده از سرویس Node](#using-nodejs)
...
ایجاد یک سرویس و ارسال پیام به کاربران شما خیلی ساده‌ست. اول از همه با اضافه کردن SDK فایبربیس به اپلیکیشنتون، کاربرانتون رو در شبکه کلاد فایبربیس رجیستر می‌کنید. و بعد در سمت سرورتون یکسری کد می‌نویسید که تا توسط اون بتونید پیام‌هاتون به کاربر یا گروهی از کاربران ارسال کنید.
...
خب پروژمون ایجاد شده و کار تمومه.با کلیک روی پروژه اپلیکیشن ایجاد شده، می‌تونیم وارد پنل بشیم و کارهای که لازمه داریم رو انجام بدیم.

<h2 id="basic-app-settings-for-fcm">تنظیمات اولیه اپلیکیشن اندروید</h2>

قبل شروع همه‌چیز این رو بگم با مراجعه به این ریپوی گیتهاب، می‌تونید به نمونه اپلیکیشنی که برای همین منظور ایجاد کردم دسترسی داشته باشید.
...
و در نهایت با باید گریدل رو Sync کنیم، بعد از اینکه Gradle کتابخونه‌های مد نظرش رو دریافت و آماده استفاده شد میریم سراغ مرحله بعد.

خب، حالا اول از همه در مسیر اپلیکیشن یک دایرکتوری به اسم `PushNotification` ایجاد میکنیم، یک کلاس به اسم `FCMInstanceIDService` ایجاد میکنیم. این کلاس وظیفه ساخت توکن و بروزرسانی اون درصورتی که اکسپایر شده رو بر عهده داره.

وقتی که کاربر یک توکن دریافت کرده یا توکن قبلی وی باطل و توکن جدید صادر میشه، تابع `onTokenRefresh` در این کلاس صدا زده میشه. ([مشاهده کلاس در گیتهاب](https://github.com/sadra/FirebaseApp/blob/master/app/src/main/java/com/isapanah/firebase/PushNotification/FCMInstanceIDService.java))
...
بعد از اینکه دو کلاس **FCMInstanceIDService** و **FCMService** وقتشه که اونهارو در `AndroidManifest` هم اونهارو به عنوان سرویس رویداد‌های `INSTANCE_ID_EVENT` و `MESSAGING_EVENT` معرفی کنیم.
...
خب قبل از اینکه بریم سراغ تنظمیات بعدی و آماده‌سازی اپلیکیشن، لازمه که یه سرویسی هم فراهم کنیم برای اینکه بتونیم با استفاده از API گوگل برای تلفن‌های همراهی که اپلیکیشن مارو نصب دارند پیام ارسال کنیم.

<h3 id="prepare-node-service">آماده سازی سرویس Node</h3>
...
در پنجره باز شده به سربرگ Cloud Messaging رفته کد Server Key رو کپی کرده و در پروژه Node بجای `API_KEY` قرار بدید.
...
<h3 id="using-nodejs">استفاده از سرویس پوش Node</h3>
...
من در این پروژه با استفاده از Express و بر پایه REST API یکسری Route آماده کردم تا شما بتونید بدون دردسر پیام‌هاتون رو ارسال کنید. ما 4 داریم که هرکدوم به ترتیب کارهای زیر رو انجام رو برعهده دارند:
...
برای استفاده از این روت‌ها می‌تونید از دستور CURL در ترمینال کمک بگیرید. اما من توصیه میکنم از [Postman](https://www.getpostman.com/) برای ریکوئست زدن استفاده کنید. کالکشن این رو ریکوئست‌هارو هم من از قبل برای شما آماده کردم و در پروژه قرار دادم. فقط کافیه از داخل دایرکتوری پروژه، در فولدر postman-export و یا [از اینجا در ریپوی گیتهاب](https://github.com/sadra/FCMPushNotification/blob/master/postman-export/Firebase%20Push%20Notification.postman_collection) اون رو دریافت کرده و به [Postmanتون import](https://www.getpostman.com/docs/postman/collections/data_formats) کنید. و یا [از اینجا](https://www.getpostman.com/collections/3776ad8010e1b7ae2c6e) بطور مستقیم به پست‌منتون اضافه کنید.

![Firebase Cloud Messaging Console](/assets/img/post/2017-10-24/fcm-8.png)

حالا تنها کاری که لازمه انجام بدید اینه که سرویس رو با `npm start` اجرا کرده و با Postman ریکوئست‌هاتون رو ارسال کنید. خب کارمون با سرویس Node و Postman تا اینجا تموم شد.

<h2 id="prepare-app-for-fcm">آماده سازی اپلیکیشن برای دریافت ناتیفیکیشن</h2>

خب رسیدیم به اصل ماجرا، حالا هم اپلیکیشن آماده است هم سرویس Node. اگه پیام رو ارسال کنید اپلیکیشن بدون هیچ مشکلی اون رو دریافت می‌کنه اما چیزی نشون نمیده، چون ما هنوز بهش نگفتیم با پیامی که دریافت کرده چه کاری بکنه.

<h3 id="receiving-notification">دریافت ناتیفیکیشن</h3>

همونطور که در قسمت قبلی گفتم پیام‌ها توسط کلاس `FCMService` در تابع `onMessageReceived` می‌تونیم دریافت کنیم. و همونطور که گفتم ۲نوع پیام داریم، یا notification هستند یا data ( و البته هردو باهم ). برای دریافت پیام notification باید تابع `getNotification` و برای data از تابع `getData` استفاده می‌کنیم.
...
من در این پروژه نمونه برای پیام data یک آبجکت بنام type تعریف کردم، اگه type برابر banner باشه، اول پیام رو دریافت کرده، و بعد یه `BigPictureStyle` برای `NotificationCompat` میسازیم، و در نهایت اون رو `NotificationCompat.Builder` نسبت‌دهی می‌کنیم.
...
private void showNotificationWithData(java.util.Map<java.lang.String,java.lang.String> body) {
    ...
    try{
        ...
        if(bodyObjects.getString("type").equals("banner")){
            showNotificationWithBanner(
              bodyObjects.getString("title"),
              bodyObjects.getString("message"),
              bodyObjects.getString("banner_url")
            );
...
من نوع پیام رو گذاشتم در حالت dialog_message تا تابع مورد نظر رو صدا کنه.
...
و بعدش در تابع `broadcastTheNotification` دیتای دریافتی رو در داخل اپلیکیشن BroadCast کردم.
...
و بعدش هم در متد `onResume` لازمه که بگم این اکتیویتی رو برای پیام برودکست شده ای که کلیدش **FCM_ACTION_NEW_NOTIFICATION** رجیستر کن.
...
توجه کنید که رجیستر کردن برای یک برادکست حتما باید بعد از ساختن BroadcastReceiver باشه.

همه چی آمادست. اگه پیام معمولی ارسال کنیم یا بصورت دیتا بنری به راحتی ناتیفیکیشن رو نشون میده.
...
```
 ...
 سرویس FCM به ما اجازه می‌ده که کاربرنامون رو در ُُTopic یا موضوع (و موضوع‌های) خاصی عضو کنیم. و بدین ترتیب پیام فقط برای کاربرانی که در فلان موضوع عضو هستند ارسال بشه. برای  در زیر پیام برای کاربرانی ارسال میشه که عضو موضوع `vip_user` هستند ارسال میشه:

```javascript
to: , "/topics/vip_user"
```

 حالا اگه بخواییم پیام برای همه کاربرا ارسال بشه چی؟ آیا باید توکن همه کابرارو لیست کنیم و بفرستیم؟ نه، نیازی به اینکار نیست! چون همه کاربرها بطور خودکار همون ابتدا عضو تاپیک `global` هستند، یعنی اگه قرار بود پیام برای همه ارسال بشه فقط کافیه بگید که:
...
- مقصد شرطی **condition**
    ...
    مقصد شرطی یا conditional به ما این امکان رو میده که پیام رو به کاربران بر اساس شرط عضوت اونها در تاپیک‌های خاصی بصورت AND و OR ارسال کنیم. برای مثال در زیر پیام برای کاربرانی که عضو VIP بوده و همچنین خانم هستند ارسال میشه:
...
<h3 id="user-topics">ارسال و دریافت ناتیفیکیشن بر اساس Topic یا موضوعی خاص</h3>
   ...
   در این پروژه ای که برای نمونه ساختم، خواستم یه پیام برای کاربرانی که درحال حاضر صفه اپلیکیشنشون باز بوده و از نوع VIP بودن ارسال کنم. پس اول از همه موقعی که اپلیکیشن لانچ شد در متد `onResume` اونهارو در تاپیک `onscreen` عضو کردم:
...
<b id="other-settings-priority">اولویت Priority:</b> شما میتونید برای پیامی که ارسال می‌کنید اولویت گذاری کنید. ما به ازای هر پیام دو اولویت داریم `normal`  و `high`.

- high: این اولویت برای مواقعی‌ست که میخواییم پیام در سریعترین زمان ممکن به دست کاربر برسه،‌و اگر اولویت همین باشه، حتی موقعی که گوشی لاک بوده و اینترنتش برای صرفه‌جویی برق قطعه، اینترنت رو وصل میکنه، پیام رو دریافت میکنه و به کاربر نشون میده. این حالت برای اپ‌هایی که پیامرسان هستن بیشتر استفاده میشه. در حالت پیشفرض برای پیام‌های notification اولویت روی high قرار داره.

- normal: این اولویت برای مواقعی‌ست که زمان رسیدن پیام اهمیت نداره،‌و به محض اینکه کاربر گوشیشو از لاک در بیاره و اینترنتش وصل بشه پیام رو بهش نشون میده. درحالت پیشفرض برای پیام‌های data اولویت روی high قرار داره.

  البته شما‌ می‌تونید خودتون بصورت دستی هم اولویت رو برای اون پیام مشخص کنید.

  برای این منظور در سرویس Nodejs و در بخش options، آیتم مربوط به priority رو اضافه می‌کنیم.
...
برای این مورد در سرویس Nodejs و در بخش options، آیتم مربوط به timeToLive رو اضافه می‌کنیم.
...
هنگامی که اپلیکیشن در حالت Lock قرار داره و پیام رو به کاربر نشون نمیده تا کاربر اپ رو باز کنه، اگر پیام جدید با همون کلید دریافت کرد،‌ پیام قبلی رو discard کرده و بجاش این پیام جدید رو استفاده می‌کنه.

برای این مورد در سرویس Nodejs و در بخش options، آیتم مربوط به collapse_key رو اضافه می‌کنیم.
...
فکر نمیکنم دیگه موردی برای گفتن مونده باشه. امیدوارم به راحتی بتونید از این سرویس استفاده کنید. اگه سوال یا مشکلی بود می‌تونید از طریق [توییتر](https://twitter.com/sadra_amlashi) یا [تلگرام](https://t.me/amlashi) با من در ارتباط باشید.
